...
async function run() {
  ...
  const [existing] = await conn.query('SELECT id FROM sys_translation WHERE name = "Enter" AND language_id = 2');
  if (existing.length === 0) {
    const currentMonthYear = new Date().toLocaleDateString('en-GB', {month: '2-digit', year: '2-digit'}).replace('/', '');
    const codeStr = `TRAN-${currentMonthYear}-9999`; // random high number
    await conn.query(`
      INSERT INTO sys_translation 
      (code, name, language_id, translated_text, is_deleted, status)
      VALUES (?, 'Enter', 2, 'Masukkan', 0, 'Saved')
    `, [codeStr]);
    console.log('Added "Enter" -> "Masukkan"');
  }
